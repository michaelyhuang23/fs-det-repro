...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq):
    ...
    for image_batch, target_batch in metric_logger.log_every(data_loader, print_freq, header):
        optimizer.zero_grad()
        loss_reduced = 0
        loss_dict_reduced = 0
        for images, targets in zip(image_batch, target_batch):
...
def coco_evaluate(model, data_loader, device):
    ...
    for image, targets in metric_logger.log_every(data_loader, 1000, header):
        image = list(img.to(device) for img in image)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
